...
// ── SA smart ID card (readable PDF417) ────────────────────────────────────────
// Unlike the licence, the smart ID card's barcode is NOT encrypted — it carries
// readable fields in this order: Surname, Names, Sex, Nationality, ID number,
// Date of birth, Country of birth, Status, Date of issue, … We locate the ID
// (hard-validated), take the alphabetic tokens that PRECEDE it as Surname + given
// Names, and derive DOB/gender from the ID itself. Token heuristics — the confirm
// screen still lets staff fix edge cases (e.g. compound surnames). Tune against a
// real scan if a specific card's field order differs.
const SID_STOPWORDS = /^(MALE|FEMALE|RSA|ZAF|SA|SOUTH|AFRICA|CITIZEN|NON|RESIDENT|PERMANENT|UNKNOWN|M|F)$/i;

function parseSmartId(text) {
  const s = String(text || '');
  const idHit = extractSaId(s);
  if (!idHit) return null;                 // no readable ID → not a decodable card here

  // Everything before the ID number holds Surname + Names (then Sex, Nationality).
  const idx = s.indexOf(idHit.idNumber);
  const before = idx > 0 ? s.slice(0, idx) : s;
  const tokens = (before.match(/[A-Za-z][A-Za-z'-]+/g) || [])
    .map(t => t.trim())
    .filter(t => t.length > 1 && !SID_STOPWORDS.test(t));

  if (!tokens.length) return null;         // ID but no readable name → let caller fall back
  return {
    idNumber : idHit.idNumber,
    lastName : tokens[0],                  // Surname is the first field
    firstName: tokens.slice(1).join(' '),  // remaining tokens are the given Names
    dob      : idHit.dob,
    gender   : idHit.gender,
    source   : 'smartid',
  };
...
function decodeScan(input = {}) {
  ...
  // Smart ID card carries readable Surname + Names + ID — parse the full record.
  if (text) {
    const sid = parseSmartId(text);
    if (sid) return sid;
  }
