...
// Railway routes IPv6 but Gmail SMTP is only reachable via IPv4.
// Resolve the hostname to its IPv4 address before creating the transporter
// so we connect directly to the IP and bypass IPv6 DNS results entirely.
async function getTransporter() {
  ...
  try {
    ...
    host = ipv4;
    console.log(`[mailer] ${smtpHost} → ${ipv4} (IPv4 forced)`);
  } catch (e) {
    console.warn('[mailer] resolve4 failed, using hostname:', e.message);
  }
